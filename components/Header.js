        ...
        <>
            <Flex p={6} gap={6} justifyContent={"flex-start"} alignItems={"center"} wrap={"wrap"}>
                ...
                <Flex gap={4}>
                    ...
                    <LinkItem text="Blog" href={"https://guchiii.hashnode.dev/"} isExternal/>
...
const LinkItem = ({ text, href, isExternal }) => {
    if (!isExternal)
        return <NextLink href={href || "/"} passHref><Link _focus={{ boxShadow: "none", }}>{text}</Link></NextLink>
    return <Link _focus={{ boxShadow: "none", }} href={href || "/"} isExternal>{text}</Link>
}   
